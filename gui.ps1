        ...
        $userInfo = Get-ADUser -identity $UserLogin.Text -ErrorAction Ignore
            if ( $null -ne $userInfo ) {
                if ( ($userInfo  | Select-Object -expand Enabled) -eq $true) {
                    ...
                    else {[System.Windows.MessageBox]::Show("Блокировка отменена")}
            } else {
            [System.Windows.MessageBox]::Show("Этот пользователь уже заблокирован")
            }
      } 
            else {
    ...
    function CheckUser  
   {
        ...
        if ($null -ne $userInfo ) {
            if([System.Windows.MessageBox]::Show('Точно?', 'Уверены?', 'YesNo','Question') -eq 'Yes'){
                ...
                [System.Windows.MessageBox]::Show("Ящик показан в адресной книге")
                } else {
                [System.Windows.MessageBox]::Show("Произошла отмена")
      }} else {
            ...
            if ($FNewPassw.Text -eq $SNewPassw.Text) {
                if ([System.Windows.MessageBox]::Show('Точно?', 'Уверены?', 'YesNo','Question') -eq 'Yes'){
                    try {@( Set-ADAccountPassword $UserLogin.Text -Reset -NewPassword (ConvertTo-SecureString -AsPlainText $FNewPassw.Text -Force -Verbose) -ErrorAction SilentlyContinue | Set-ADuser -ChangePasswordAtLogon $True )
                        [System.Windows.MessageBox]::Show("Пароль пользователя обновлён")}
                    catch {[System.Windows.MessageBox]::Show("Произошла неизвестная ошибка")} }
                else {[System.Windows.MessageBox]::Show("Смена пароля отменена")}
         } else {[System.Windows.MessageBox]::Show("Пароли не совпадают")}
      }
       else {
        [System.Windows.MessageBox]::Show("Пользователь не найден")
      }
...
function CreateUserFunc { 
    function CheckUser     {
        ...
        if([System.Windows.MessageBox]::Show($message,'Подтверждение', 'YesNo','Question') -eq 'Yes'){
            try {New-ADUser -Name $userName -UserPrincipalName $UPName -Department $departmentName -GivenName $firstName  -Surname $lastName -OtherName $appointmentName -SamAccountName $userName -City $cityName -Company $companyName -title $appointmentName -Enabled $true
    ...
    $LabelAppointment.text                      = "Выберите отдел"
...
function ConnectToDBFunc {
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show("Поздравляем","Вам удалось авторизоваться в exchange")
        $pass = $true
        $StartForm.Controls.Remove($ConnectToDBBtn)
        $StartForm.Controls.Add($AddToDBBtn)
      } catch {
        [System.Windows.MessageBox]::Show("Произошла ошибка","Вы не смогли авторизоваться в Exchange.")
      }
    }
    $CurrForm                    = New-Object system.Windows.Forms.Form
    $CurrForm.ClientSize         = '500,300'
...
function AddToDBFunc {
    ...
    $userInfo = Get-ADUser -identity $UserLogin.Text -ErrorAction Ignore
            if ( $null -ne $userInfo ) {
                ...
                [System.Windows.MessageBox]::Show("Почтовый ящик создан")
            } 
            else {
        [System.Windows.MessageBox]::Show("Невозможно найти пользователя $UserLogin.Text")
           }
